...
/*!50001 VIEW `orderhistory` AS select `customers`.`CustomerID` AS `CustomerID`,`customers`.`FullName` AS `FullName`,`orders`.`OrderID` AS `OrderID`,`orders`.`Cost` AS `Cost`,`menu`.`MenuName` AS `Menu`,`menuitems`.`CourseName` AS `CourseName`,`menuitems`.`StarterName` AS `StarterName` from (((`customers` join `orders` on((`customers`.`CustomerID` = `orders`.`CustomerID`))) join `menu` on((`orders`.`MenuID` = `menu`.`MenuID`))) join `menuitems` on((`menu`.`MenuItemsID` = `menuitems`.`MenuItemsID`))) order by `orders`.`Cost` */;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `AddValidBooking`(IN book_b DATETIME, IN table_b INT, IN Cust_b INT)
BEGIN
START TRANSACTION;
INSERT INTO Bookings (TableNum, BookDate, CustomerID) VALUES (table_b, book_b, Cust_b);
IF (SELECT COUNT(BookingID) FROM Bookings WHERE BookDate = book_b AND TableNum = table_b > 1)
	THEN
    COMMIT;
    SELECT "Booking Added";
    ELSE
    ROLLBACK;
    SELECT "Table Already Booked";
END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
...
/*!50003 SET sql_mode              = 'ONLY_FULL_GROUP_BY,STRICT_TRANS_TABLES,NO_ZERO_IN_DATE,NO_ZERO_DATE,ERROR_FOR_DIVISION_BY_ZERO,NO_ENGINE_SUBSTITUTION' */ ;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` PROCEDURE `CheckBooking`(book_a DATETIME, table_a INT)
BEGIN
IF (SELECT EXISTS (SELECT * FROM Bookings WHERE BookDate = book_a AND TableNum = table_a) )
THEN ( SELECT CONCAT("Table", " ", table_a, " ", "is booked") AS "Booking Status" );
ELSE ( SELECT CONCAT("Table", " ", table_a, " ", "is available") AS "Booking Status" );
END IF;
END ;;
DELIMITER ;
/*!50003 SET sql_mode              = @saved_sql_mode */ ;
